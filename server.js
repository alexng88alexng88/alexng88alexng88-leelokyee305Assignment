    ...
    if (request.method == "POST") {
        ...
        return request.on("data", function(data) {
			formData += data;
				...
				console.log("Lok Yee Assignment Shows : "+msg);
					
					obj = JSON.parse(msg);
				  console.log("Status is : "+obj['act']);	
					if (request.url == "/login.html") {
						 console.log("Congrats! You have been successfully Login!");
...
//	response.end('{"success" : "Updated Successfully", "status" : 200}');
				...
				//		db.collection("user", function(err, collection) {
					...
					//		}, function(err, data) {
							...
							//	if (data) {
									console.log("Congrats! U sign in successfully!");
